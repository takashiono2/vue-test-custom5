<template>
  <div>
    <v-app-bar color="primary" dark app clipped-left><!--clippedあると、ナビバーを巻き込まない-->
      ...
      <v-spacer />
      <!-- <div>{{userName}}</div> -->
      <div>{{setDisplayName}}</div>
      <img v-if="loginUser" :src="setPhotoURL">
      <!-- {{this.user.photoURL}} -->
  ...
  data(){
    return {
        items:[
          ...
          {icon: 'mdi-book-open-blank-variant',title: 'アドレス登録',link:'/addressform'},
          {icon: 'mdi-book-open-variant',title: 'アドレス',link:'/addresses'}
        ],
    }
  },
  computed: {
    ...mapGetters(['setPhotoURL','loginUser','setDisplayName']),
